<h1 align="center">👋 Hi, I'm AgentForge</h1>

<h3 align="center">
  AI/ML & Full-Stack Engineer | Agentic AI • LLM Systems • RAG • Distributed Systems
</h3>

<p align="center">
  Building intelligent systems from idea → architecture → production 🚀
</p>

<!-- GitHub Contribution Snake -->
<!-- GitHub Contribution Snake -->

<p align="center">
  <picture>
    <source
      media="(prefers-color-scheme: dark)"
      srcset="https://raw.githubusercontent.com/agentforge9/agentforge9/gh-pages/github-contribution-grid-snake-dark.svg"
    />
    <source
      media="(prefers-color-scheme: light)"
      srcset="https://raw.githubusercontent.com/agentforge9/agentforge9/gh-pages/github-contribution-grid-snake.svg"
    />
    <img
      alt="GitHub contribution snake"
      src="https://raw.githubusercontent.com/agentforge9/agentforge9/gh-pages/github-contribution-grid-snake.svg"
    />
  </picture>
</p>

---

## 👨‍💻 About Me

- 🤖 I build <strong>production AI/ML and agentic systems</strong> — from experimentation and orchestration to APIs, deployment, evaluation, and observability.

- 🧠 Working with <strong>LLMs, RAG, AI agents, tool/function calling, multi-agent workflows, semantic search, and autonomous systems</strong>.

- ⚙️ Experienced in building <strong>scalable backend platforms</strong> using Python, FastAPI, Java, Node.js, REST/GraphQL APIs, microservices, and event-driven architectures.

- 🌐 Building modern <strong>full-stack applications</strong> with React, Next.js, TypeScript, Node.js, and cloud-native services.

- ☁️ Experienced with <strong>AWS, Azure, GCP, Docker, Kubernetes, CI/CD, distributed systems, and production infrastructure</strong>.

- 🔬 Interested in turning emerging AI research into <strong>reliable real-world software</strong> rather than stopping at prototypes.

- 🌎 I enjoy <strong>open source, learning new technologies, sharing ideas, and making friends with developers around the world</strong>.

- ⚡ Motto: <em>"What you do today can improve all of your tomorrows."</em>

<h4 align="center">
  🤝 Let's build something interesting — fork a project, ⭐ star a repo, or reach out!
</h4>

---

## 🚀 What I Build

<table>
<tr>
<td width="50%" valign="top">

### 🤖 AI Engineering

- Agentic AI & Autonomous Workflows
- LLM Applications & Orchestration
- Retrieval-Augmented Generation (RAG)
- Tool / Function Calling
- Multi-Agent Systems
- AI Evaluation & Observability
- Semantic & Vector Search
- Recommendation Systems
- ML/LLM Production Pipelines

</td>
<td width="50%" valign="top">

### 💻 Software Engineering

- AI-Powered Full-Stack Applications
- Distributed Backend Systems
- REST & GraphQL APIs
- Microservices Architecture
- Event-Driven Systems
- Real-Time Applications
- Cloud-Native Platforms
- CI/CD & Production Automation
- System Architecture & Performance

</td>
</tr>
</table>

---

# 🧠 AI / LLM Stack

<p align="center">

<img src="https://github.com/lucyhunter9411/profile_icon/blob/main/AI/openai.png?raw=true" height="45" alt="OpenAI" />
&nbsp;&nbsp;
<img src="https://github.com/lucyhunter9411/profile_icon/blob/main/AI/claude.png?raw=true" height="45" alt="Claude" />
&nbsp;&nbsp;
<img src="https://github.com/lucyhunter9411/profile_icon/blob/main/AI/huggingface.png?raw=true" height="45" alt="Hugging Face" />
&nbsp;&nbsp;
<img src="https://github.com/lucyhunter9411/profile_icon/blob/main/AI/langchain.png?raw=true" height="45" alt="LangChain" />
&nbsp;&nbsp;
<img src="https://github.com/lucyhunter9411/profile_icon/blob/main/AI/pinecone.png?raw=true" height="45" alt="Pinecone" />
&nbsp;&nbsp;
<img src="https://github.com/lucyhunter9411/profile_icon/blob/main/AI/faiss.png?raw=true" height="45" alt="FAISS" />

</p>

<p align="center">
LLMs • RAG • Agents • Embeddings • Vector Search • Prompt Engineering • Evaluation • AI Orchestration
</p>

---

# 💻 Languages

<p align="center">

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="45" alt="Python" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" height="45" alt="Java" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="45" alt="JavaScript" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" height="45" alt="TypeScript" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" height="45" alt="C#" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" height="45" alt="PHP" />

</p>

---

# 🌐 Full-Stack Engineering

<p align="center">

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" height="45" alt="React" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" height="45" alt="Next.js" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" height="45" alt="Node.js" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/fastapi/fastapi-original.svg" height="45" alt="FastAPI" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/spring/spring-original.svg" height="45" alt="Spring" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/dot-net/dot-net-original.svg" height="45" alt=".NET" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/laravel/laravel-original.svg" height="45" alt="Laravel" />

</p>

<p align="center">
React • Next.js • Node.js • FastAPI • Spring Boot • .NET • REST • GraphQL
</p>

---

# ☁️ Cloud & DevOps

<p align="center">

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" height="50" alt="AWS" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/azure/azure-original.svg" height="45" alt="Azure" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/googlecloud/googlecloud-original.svg" height="45" alt="Google Cloud" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" height="45" alt="Docker" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kubernetes/kubernetes-original.svg" height="45" alt="Kubernetes" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/githubactions/githubactions-original.svg" height="45" alt="GitHub Actions" />

</p>

<p align="center">
AWS • Azure • GCP • Docker • Kubernetes • CI/CD • Cloud Architecture
</p>

---

# 🗄️ Data & Infrastructure

<p align="center">

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" height="45" alt="PostgreSQL" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" height="45" alt="MongoDB" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" height="45" alt="MySQL" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/redis/redis-original.svg" height="45" alt="Redis" />
&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/apachekafka/apachekafka-original.svg" height="45" alt="Kafka" />

</p>

<p align="center">
PostgreSQL • MongoDB • MySQL • Redis • Kafka • Vector Databases
</p>

---

# ⚡ Engineering Focus

```text
AI Agents              ████████████████████
LLM / RAG Systems       ████████████████████
Python / FastAPI        ███████████████████░
React / TypeScript      ███████████████████░
System Architecture     ███████████████████░
Cloud / Distributed     ██████████████████░░Built with collaboration and open-source learning.

## Collaboration
Built with collaboration and open-source learning.
